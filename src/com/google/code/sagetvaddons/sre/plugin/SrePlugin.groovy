...
public final class SrePlugin extends AbstractPlugin {
	...
	void start() {
		...
		addProperty(p)
		p = new ServerStoredProperty(CONFIG_BOOL, PROP_LIVE_ONLY, 'false', 'Only monitor airings that are makred as live in the EPG data unless an override is defined for the airing.')
		addProperty(p)
		...
		addProperty(p)
		p = new ServerStoredProperty(CONFIG_INTEGER, PROP_MAX_EXTENSION, '8', 'Maximum Extention Time (hours)', '', new IntegerRangeValidator(1, 16))
		addProperty(p)
